...
CMD ["sh", "-c", "composer install --no-interaction --prefer-dist --no-progress && npm install --prefer-offline --no-audit --no-fund && npm run build && mkdir -p storage/framework/{cache,sessions,views} storage/logs bootstrap/cache && chmod -R 775 storage bootstrap/cache && php artisan config:clear && php artisan migrate --force && php artisan serve --host=0.0.0.0 --port=8007"]
